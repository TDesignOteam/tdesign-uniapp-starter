...
const totalSituationDataList = ref<DataItem[]>([]);
const interactionSituationDataList = ref<DataItem[]>([]);
const completeRateDataList = ref<ProgressItem[]>([]);

const getMemberData = async () => {
  const res: any = await request('/dataCenter/member');
  totalSituationDataList.value = res.data.template.succ.data.list;
};

const getInteractionData = async () => {
  const res: any = await request('/dataCenter/interaction');
  interactionSituationDataList.value = res.data.template.succ.data.list;
};

const getCompleteRateData = async () => {
  const res: any = await request('/dataCenter/complete-rate');
  completeRateDataList.value = res.data.template.succ.data.list;
};
